...
notstereo='6 channels';

# Faz as conversoes 
if [ "$mediainforesult" = "(MKVInfo) No EBML head found." ]; then
	# verifica se a trilha atual já é AAC Stereo - se nao for, converte
    if [[ "$mediainforesult2" == *"$notstereo"* ]]; then
    ...
    trackDTS=$(cat .mediainfo 2>&1 | grep -B3 A_DTS | awk '{print substr($0,index($0,"mkvextract:")+12,1)}' | head -n 1)
    trackAC3=$(cat .mediainfo 2>&1 | grep -B3 A_AC3 | awk '{print substr($0,index($0,"mkvextract:")+12,1)}' | head -n 1)
    ...
    ffmpeg -i "$ac3file" -acodec libfaac -ac 2 "$aacfile"
...
#    echo ffmpeg -i $movie -i $aacfile -vcodec copy -acodec copy $moviem4v -map 0.0 -map 1.0 -threads 4
    ffmpeg -i "$movie" -i "$aacfile" -vcodec copy -acodec copy "$moviem4v" -map 0.0 -map 1.0 -newaudio -threads 4
fi

echo 'Finished remuxing!'

pwd > .curdir
currentDir=$(cat .curdir)

osascript -e "tell application \"Finder\" to set label index of alias POSIX file \"$currentDir\" to 6"	

growlnotify -m "Finished encoding movie $movie" -n "ATV Media Converter"
